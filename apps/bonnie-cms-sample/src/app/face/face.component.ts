...
@Component({
	...
	providers: [{ provide: LocalizeServiceBase, useClass: RouterLocalizeService }],
})
export default class FaceComponent {
	...
	constructor() {
		effect(() => {
			...
			const localizedOgImage = this.localizeService.getLocalized(
				newPage.openGraph.image,
				undefined,
			);

			if (localizedOgImage !== undefined) {
				const ogImage =
					localizedOgImage.url.startsWith("http") || localizedOgImage.url.startsWith("//")
			...
			const localizedOgDescription = this.localizeService.getLocalizedText(
				newPage.openGraph.description,
			);
		...
		effect(() => {
			...
			if (newSettings[CmsSampleSettingIds.faviconIco]) {
				this.appendIconLink(
					"icon",
					"image/x-icon",
					(newSettings[CmsSampleSettingIds.faviconIco] as FileSettingData).src.url,
				);
			}
			...
			if (newSettings[CmsSampleSettingIds.faviconSvg]) {
				this.appendIconLink(
					"icon",
					"image/svg+xml",
					(newSettings[CmsSampleSettingIds.faviconSvg] as SvgSettingData).src.url,
				);
			}
			...
			if (newSettings[CmsSampleSettingIds.faviconAppleTouch180]) {
				this.appendIconLink(
					"apple-touch-icon",
					"image/png",
					(newSettings[CmsSampleSettingIds.faviconAppleTouch180] as OneImageSettingData).src.url,
				);
			}

			if (newSettings[CmsSampleSettingIds.description]) {
				const localizedDescription = this.localizeService.getLocalizedText(
					(newSettings[CmsSampleSettingIds.description] as LocalizedTextSettingData).text,
				);
