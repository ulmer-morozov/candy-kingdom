import { HostBinding, Component, effect, output, model, signal, input, computed } from '@angular/core';
...
export abstract class BoneEditorBaseComponent<TBone extends Bone> implements IBoneEditor<TBone> {
  ...
  private updatePresetByData = (): void => {
    const countOfActive = this.presets().map(p => p.isActive(this.bone())).filter(p => p).length;
