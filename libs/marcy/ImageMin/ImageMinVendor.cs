...
public abstract class ImageMinVendor
{
    ...
    public abstract Task Minify(string input, string output, CancellationToken cancellationToken = default);
    public abstract bool CanBeApplyed(string format);

    public async Task<MemoryStream> Minify(string input, CancellationToken cancellationToken = default)
    {
